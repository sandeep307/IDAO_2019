...
def load_train_test_csv(path):
    train = pd.concat([
        pd.read_csv(os.path.join(path, "train_part_%i_%s.csv.gz" % (i, VERSION)), nrows = 100,
                    index_col=ID_COLUMN)
        for i in (1, 2)], axis=0, ignore_index=True)
    test = pd.read_csv(os.path.join(path, "test_public_%s.csv.gz" % VERSION), nrows = 100, index_col=ID_COLUMN)
    return train, test
